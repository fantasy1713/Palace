...
public class HibernateDao {
	...
	private void setParameter(Query query, Object... parameter) {
		if (parameter != null) {
			for (int i = 0; i < parameter.length; i++) {
				if (query instanceof org.hibernate.query.internal.QueryImpl)
	 ...
	 * getFullTextSession().createIndexer(Article.class).startAndWait(); } catch
	 ...
	 * @return 分页对象
	 * 
	 * 		@SuppressWarnings("unchecked") public Page search(Page page,
	 *         BooleanQuery query, BooleanQuery queryFilter, Sort sort, Class
	 *         <?> ... clauses){
	 * 
	 *         // 按关键字查询 FullTextQuery fullTextQuery = null; if(clauses == null)
	 *         fullTextQuery = getFullTextSession().createFullTextQuery(query,
	 *         entityClass); else fullTextQuery =
	 *         getFullTextSession().createFullTextQuery(query, clauses);
	 * 
	 *         // 过滤无效的内容 fullTextQuery.setFilter(new CachingWrapperFilter(new
	 *         QueryWrapperFilter(queryFilter)));
	 * 
	 *         // 按时间排序 fullTextQuery.setSort(sort);
	 * 
	 *         // 定义分页 page.setCount(fullTextQuery.getResultSize());
	 ...
	 * 
	 *         // 返回结果 page.setList(fullTextQuery.list());
	 * 
	 *         return page; }
	 */
	/**
	 ...
	 *         BooleanQuery(); try { if (StringUtils.isNotBlank(q)){ for (String
	 *         field : fields){ QueryParser parser = new
	 *         QueryParser(Version.LUCENE_36, field, analyzer);
	 *         query.add(parser.parse(q), Occur.SHOULD); } } } catch
	 ...
	 *            字段名
	 * 
	 *            public <T extends BaseEntity> List
	 *            <T> keywordsHighlight(BooleanQuery query, List<T> list,
	 *            String... fields){ Analyzer analyzer = new IKAnalyzer();
	 *            Formatter formatter = new SimpleHTMLFormatter(
	 *            "<span class=\"highlight\">", "</span>"); Highlighter
	 *            highlighter = new Highlighter(formatter, new
	 *            QueryScorer(query)); highlighter.setTextFragmenter(new
	 *            SimpleFragmenter(100)); for(T entity : list){ try { for
	 *            (String field : fields){ String text =
	 *            StringUtils.replaceHtml((String)Reflections.invokeGetter(
	 *            entity, field)); String desciption =
	 *            highlighter.getBestFragment(analyzer,field, text);
	 *            if(desciption!=null){ Reflections.invokeSetter(entity,
	 *            fields[0], desciption); break; }
	 *            Reflections.invokeSetter(entity, fields[0],
	 *            StringUtils.abbreviate(text, 100)); }
	 *            //Reflections.invokeSetter(entity, fields[1],
	 *            "sdfkjsdlkfjklsdjf"); } catch (IOException e) {
